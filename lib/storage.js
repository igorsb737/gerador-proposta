...
if (useBlob) {
  try {
    ...
    blobDriver = {
      ...
      async listPropostas() {
        try {
          console.log('=== INÍCIO listPropostas() ===');
          console.log('Chamando list() com prefix: propostas/');
          
          const result = await list({ prefix: 'propostas/' });
          console.log('Resultado do list():', {
            hasBlobs: !!result.blobs,
            blobsLength: result.blobs?.length || 0,
          ...
          if (blobs.length === 0) {
            console.log('AVISO: Nenhum blob encontrado com prefix propostas/');
            return [];
          }
          
          console.log('Blobs encontrados:', blobs.map(b => ({
            pathname: b.pathname,
            size: b.size,
          ...
          for (let i = 0; i < blobs.length; i++) {
            const b = blobs[i];
            try {
              console.log(`[${i+1}/${blobs.length}] Processando:`, b.pathname);
              
              const resp = await fetch(b.url);
              console.log(`[${i+1}/${blobs.length}] Fetch status:`, resp.status, resp.statusText);
              
              if (!resp.ok) {
                console.error(`[${i+1}/${blobs.length}] ERRO: Blob inacessível`, b.pathname, resp.status);
                continue;
              }
              
              const text = await resp.text();
              console.log(`[${i+1}/${blobs.length}] Conteúdo recebido, tamanho:`, text.length);
              
              if (!text.trim()) {
                console.error(`[${i+1}/${blobs.length}] ERRO: Conteúdo vazio`);
                continue;
              }
              
              const d = JSON.parse(text);
              console.log(`[${i+1}/${blobs.length}] JSON parseado:`, {
                id: d.id,
                cliente: d.cliente,
                data: d.data,
                plano: d.plano
              ...
              console.error(`[${i+1}/${blobs.length}] ERRO ao processar:`, b.pathname, error.message);
            }
          }
          
          console.log('=== RESULTADO FINAL ===');
          console.log('Total processado:', out.length, 'de', blobs.length, 'blobs');
          console.log('Lista final:', out);
          console.log('=== FIM listPropostas() ===');
          
          return out;
          
        } catch (error) {
          console.error('ERRO CRÍTICO em listPropostas():', error.message);
