    ...
    blobDriver = {
      ...
      async listPropostas() {
        ...
        for (const b of blobs) {
          try {
            console.log('Fazendo fetch do blob:', b.pathname, 'URL:', b.url);
            
            // Usar a URL direta do blob (que já é pública)
            const resp = await fetch(b.url);
            if (!resp.ok) {
              console.warn('Blob não acessível:', b.pathname, 'Status:', resp.status, 'URL:', b.url);
              continue;
            }
            
            const text = await resp.text();
            console.log('Conteúdo bruto do blob:', b.pathname, '-> Tamanho:', text.length);
            
            const d = JSON.parse(text);
            console.log('Dados parseados:', b.pathname, '-> ID:', d.id, 'Cliente:', d.cliente);
            out.push({ id: d.id, cliente: d.cliente, data: d.data, plano: d.plano });
          } catch (error) {
            console.error('Erro ao processar blob:', b.pathname, 'Erro:', error.message);
          }
        }
        console.log('Lista final de propostas do Blob:', out.length, 'itens');
        return out;
      }
    };
  } catch (e) {
